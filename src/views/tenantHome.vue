...
<script>
// @ is an alias to /src
import Slider from '@/components/tenantSlider.vue'
...
export default {
    ...
    tabRemove (targetName) {
      ...
      if (this.activeIndex === targetName) {
        ...
        if (this.openTab && this.openTab.length >= 1) {
          ...
          this.$router.push({path: 'tenantInfo'})
        }
      }
    }
  },
  mounted () {
    ...
    if (this.$route.path !== '/tenantInfo' && this.$route.path !== '/tenantHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/tenantInfo', name: '个人信息'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/tenantInfo', name: '个人信息'})
      this.$store.commit('set_active_index', '/tenantInfo')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/tenantHome' && to.name === undefined) {
          return
        }
